...
class uav_setpoints:
    def __init__(self) -> None: #初始化
        self.circle_setpoint_moveToPositionAsync = (   #预设圈的位置和运行速度（x,y,z,v)
            (15.5, -19.6 , -3.5, 5),
            (22, -41.2 , -2.5, 5),
            (21, -61.5 , -2, 3),
            (10, -78.2, -2, 3),
        ...
        self.circle_yaw_rotateToYawAsync = ( #预设绕y轴旋转的度数
            -90,
            -90,
            -100,
            -120,
...
class airsim_client:
    def __init__(self, ip_addr='127.0.0.1') -> None: #id用于连接场景
        ...
        self.circle_finder = circle_finder.circle_finder(self.client) #让circle.finder()初始化

        self.setpoints = uav_setpoints()

    def task_takeoff(self):
        self.client.armDisarm(True)
        self.client.takeoffAsync().join() #.join()是为了让飞机先起飞然后才进行下一个动作

    def task_cross_circle(self, circle_id_from_one): #输入圈的序号
        self.client.moveToPositionAsync(*self.setpoints.get_circle_setpoint(circle_id_from_one)).join() #输入圈的坐标位置
