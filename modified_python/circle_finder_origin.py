...
class circle_finder:
    ...
    def get_uav_position_rotation_in_wc(self):
        ...
        quaternionr = state.kinematics_estimated.orientation  #获取无人机朝向
        w = quaternionr.w_val
        x = quaternionr.x_val
        y = quaternionr.y_val
        z = quaternionr.z_val
        ...
        position = state.kinematics_estimated.position  #获取无人机位置
        position_list = []
    ...
    def get_rgb_depthperspective_image(self):   #获取rgb图和深度图
    ...
    def get_circle_x_y(self, rgb, depthperspective):
        depthperspective[depthperspective > 8] = 0   #深度图里大于8的值设置为0
        depthperspective = depthperspective.astype(np.uint8)  #窗口显示前要转换格式
        depthperspective = cv2.equalizeHist(depthperspective)
        # print(depthperspective)
        cv2.imshow('depth', depthperspective)  #窗口显示深度图
        cv2.waitKey(1000)   #参数为窗口显示时间，0表示一直显示直到手动关闭
        ...
        cv2.imshow('new', rgb_pic)  #窗口显示rgb图（并已标出⚪和圆心）
        cv2.waitKey(1000)   #参数为窗口显示时间，0表示一直显示直到手动关闭
